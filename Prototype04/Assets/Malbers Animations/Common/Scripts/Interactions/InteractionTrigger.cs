...
public class InteractionTrigger : MonoBehaviour
{
    ...
    [Tooltip("当前交互状态")]
    [SerializeField] InteractionState currentState = InteractionState.Ready;
    ...
    public class DialogueMessage
    {
        ...
        public int noResourceDialogueIndex = -1; // -1表示结束对话
        public bool isNoResourceMessage = false;
    ...
    public class DialogueChoice
    {
        ...
        public List<string> narrativeActionIDs = new List<string>();
            [Header("资源不足处理")]
    ...
    private void Start()
    {
        ...
        RestoreNarrativeActionReferences();
...
private void RestoreNarrativeActionReferences()
{
    // 查找场景中所有NarrativeAction
       NarrativeAction[] allActions = FindObjectsOfType<NarrativeAction>();
    
    if (allActions.Length == 0)
    {
        Debug.LogWarning("在场景中未找到任何NarrativeAction组件");
        return;
    }
    
    Debug.Log($"找到 {allActions.Length} 个NarrativeAction，可供使用");
}

    // 保存当前引用的ID
private void SaveNarrativeActionIDs(List<NarrativeAction> actions, List<string> idList, string typeLabel)
{
    // 如果已经有ID，就不需要再次保存
    if (idList.Count > 0)
    {
    ...
    // 从现有引用中获取ID (可能用于编辑器模式或从旧版本迁移)
    foreach (var action in actions)
    {
        if (action != null && !string.IsNullOrEmpty(action.actionID))
        {
            // 避免重复
            if (!idList.Contains(action.actionID))
            {
                idList.Add(action.actionID);
                Debug.Log($"保存{typeLabel}叙事动作ID：{action.actionID}");
            }
        }
        else if (action != null)
        {
            Debug.LogWarning($"{typeLabel}叙事动作 {action.name} 没有设置actionID，无法保存引用");
        }
    }
}

private void RestoreActionsByIDs(NarrativeAction[] allActions, List<string> idList, List<NarrativeAction> targetList, string typeLabel)
{
    foreach (string actionID in idList)
    {
        if (!string.IsNullOrEmpty(actionID))
        {
            // 查找匹配ID的NarrativeAction
            NarrativeAction foundAction = System.Array.Find(allActions, 
                action => action.actionID == actionID);
                
            if (foundAction != null)
            {
                targetList.Add(foundAction);
                Debug.Log($"已恢复{typeLabel}叙事动作引用：{actionID}");
            }
            else
            {
                Debug.LogWarning($"未找到ID为 {actionID} 的{typeLabel}叙事动作");
            }
        }
    }
}
    ...
    {
        // 如果玩家死亡或者处于冷却状态，直接返回
        if (isPlayerDead || currentState == InteractionState.Cooldown || currentState == InteractionState.Completed)
            return;
            ...
            if (interactionKeyPressed)
            {
                ...
                if (currentState == InteractionState.Active)
                {
                    ...
                    if (enableDialogue)
                    {
                        ...
                        if (message.isNoResourceMessage)
                        {
                            ExitInteraction();  // 玩家按下键才退出
                            return;
                        }
...
private void ExecuteNoResourceActions(DialogueChoice choice)
{
    // 找到并执行所有资源不足叙事动作
    NarrativeAction[] allActions = FindObjectsOfType<NarrativeAction>();
    foreach (string id in choice.noResourceNarrativeActionIDs)
    {
        if (!string.IsNullOrEmpty(id))
        {
            var action = System.Array.Find(allActions, x => x.actionID == id);
            if (action != null)
            {
            ...
            {
                Debug.LogWarning($"未找到ID为 {id} 的资源不足叙事动作");
            }
        }
    }
}
...
private void HandleResourceChoice(DialogueChoice choice)
{
    ...
    if (choice.consumeResources)
    {
        ...
        if (resourceManager == null || !resourceManager.isPickedUp)
        {
            ...
            // 执行资源不足narrative actions
            ExecuteNoResourceActions(choice);
            ...
            {
                // 未设置 index，显示默认文本，但不立即退出
                StartCoroutine(FadeText("你没有足够的资源来进行此操作。", TextType.Dialogue));
                if (dialogueSettings.backgroundImage != null)
                {
                    StartCoroutine(FadeTextBackground(1.0f));
                }
        ...
        // 正常进入下一条消息
        JumpToNextMessage(choice);
        pendingCompleteAfterExit = true;  // 保留这个标志，确保在对话结束后交互完成
    }
    else
    {
        // 不需要资源消耗的选项，原样处理
    ...
    private void ExecuteChoiceActions(DialogueChoice choice)
    {
        ...
        // 找到并执行所有叙事动作
        NarrativeAction[] allActions = FindObjectsOfType<NarrativeAction>();
        foreach (string id in choice.narrativeActionIDs)
        {
            if (!string.IsNullOrEmpty(id))
            {
                var action = System.Array.Find(allActions, x => x.actionID == id);
                ...
                {
                    Debug.LogWarning($"未找到ID为 {id} 的叙事动作");
                }
            }
        }
...
private void JumpToResourceFailureMessage(DialogueMessage currentMessage)
{
    ...
    {
        Debug.Log($"[资源不足] 跳转到资源不足对话索引: {currentMessage.noResourceDialogueIndex}");
    ...
    {
        Debug.LogWarning("[资源不足] 未设置有效的资源不足对话索引，使用默认提示");
    ...
    private void DisplayCurrentMessage()
    {
        ...
        else
        {
            ...
            // ✅ 如果不是资源不足这种 pending 退出的情况，再更新 Prompt
            if (!pendingCompleteAfterExit && !message.isNoResourceMessage)
            {
                UpdatePromptMessage();
            }
        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag(playerTag) && !isPlayerDead)
        {
            ...
            if (currentState == InteractionState.Completed)
            {
                return; // ✅ 如果已完成，不再响应碰撞进入
            }
    ...
    {
            if (currentState == InteractionState.Completed) return; // ✅ 禁止已完成状态下提示出现
...
{
    // 稍微延迟一下，确保所有对象都已加载
    Invoke("RestoreNarrativeActionReferences", 0.1f);
}
}
